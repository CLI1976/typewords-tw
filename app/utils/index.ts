...
export function checkAndUpgradeSaveDict(val: any) {
  ...
  if (val) {
    try {
      ...
      if (!data.version) {
        console.warn('資料缺少版本號，回傳預設狀態')
        return defaultState
      }
      let state: any = data.val
      if (typeof state !== 'object') {
        console.warn('資料格式無效，回傳預設狀態')
        return defaultState
      }
      state.load = false
      ...
      if (version === SAVE_DICT_KEY.version) {
        ...
        // 版本不匹配時，盡量保留資料而不是直接回傳預設狀態
        console.warn(`資料版本不匹配: 目前版本 ${version}, 期望版本 ${SAVE_DICT_KEY.version}，嘗試保留資料`)
        try {
          checkRiskKey(defaultState, state)
          // 嘗試保留 bookList 資料
          ...
          console.error('資料升級失敗，回傳預設狀態', upgradeError)
          return defaultState
        }
      }
    } catch (e) {
      console.error('資料解析異常，回傳預設狀態', e)
      return defaultState
    }
  }
  return defaultState
  ...
  if (val) {
    try {
      ...
      if (version === SAVE_SETTING_KEY.version) {
        ...
        // 為了保持永遠是最新的快捷鍵選項列表，但保留住使用者的自訂設定，去掉無效的快捷鍵選項
...
// 篩選未自訂的詞典，未自訂的詞典不需要儲存單字，用的時候再下載
...
export function shuffle<T>(array: T[]): T[] {
  ...
  for (let i = result.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1)) // 產生 0 ~ i 的隨機索引
...
export function getRandomN(arr: any[], n: number) {
  ...
  for (let i = copy.length - 1; i > 0; i--) {
    ...
    ;[copy[i], copy[j]] = [copy[j], copy[i]] // 交換
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    if (url.includes('.mjs')) {
      ...
      script.onload = async () => {
        try {
          // 使用動態 import 載入模組
          const module = await import(/* @vite-ignore */ url) // dynamic import .mjs module
          window[key] = module.default || module // 將模組掛到 window 物件
          resolve(window[key])
        } catch (err) {
          reject(`${key} 載入失敗: ${err.message}`)
        }
      }
    } else {
    ...
    script.onerror = () => reject(key + ' 載入失敗')
