...
function getPlanButtonText(plan: Plan) {
  if (plan.id === selectedPlanId) return '已選中'
  if (plan.id === currentPlan?.id) return '目前方案'
  return '選擇'
}
...
async function getCouponInfo() {
  if (showCouponInput) {
    if (!coupon.code) return Toast.info('請輸入優惠券')
  ...
  <BasePage>
    <div class="space-y-6">
      ...
      <div v-if="member?.active" class="card-white bg-green-50 dark:bg-item border border-green-200 mt-3 mb-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <IconFluentCheckmarkCircle20Regular class="mr-2 text-green-600" />
            <div>
              <div class="font-semibold text-green-800">目前方案：{{ currentPlan?.name }}</div>
          ...
          <div class="text-align-end space-y-2">
            <div v-if="member.autoRenew" class="flex items-center gap-space">
              ...
              <PopConfirm title="確認取消？" @confirm="toggleAutoRenew">
      ...
      <div class="flex justify-between">
        <div class="title">選擇适合您的套餐</div>
        <div class="subtitle">三种方案，按需選擇</div>
      </div>
    ...
    <div id="pay" class="mb-50" v-if="selectedPlanId">
      ...
      <div class="grid grid-cols-3 gap-8">
        ...
        <div class="card-white">
          <div class="text-lg font-medium mb-4">選擇支付方式</div>
        ...
        <div class="card-white">
          ...
          <div class="bg-second text-sm px-4 py-3 rounded-lg mb-4 text-gray-600">
            會員屬於虛擬服務，一經購買啟用後不支援退款。請在購買前仔細閱讀權益說明，確認符合您的需求再進行支付。
          </div>
