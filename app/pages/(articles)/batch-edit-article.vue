...
function importData(e: any) {
  ...
  reader.onload = async function (s) {
    ...
    if (res.length) {
        ...
        .map(v => {
          if (v['原文標題'] && v['原文正文']) {
            return getDefaultArticle({
              id: nanoid(6),
              title: String(v['原文標題']),
              titleTranslate: String(v['譯文標題']),
              text: String(v['原文正文']),
              textTranslate: String(v['譯文正文']),
              audioSrc: String(v['音訊網址']),
            })
          }
        })
        .filter(v => v)
      ...
      if (repeat.length) {
        MessageBox.confirm(
          '文章「' + repeat.map(v => v.title).join(', ') + '」已存在，是否覆蓋原有文章？',
          '偵測到重複文章',
          () => {
            repeat.map(v => {
...
async function exportData(val: { type: string; data?: Article }) {
  ...
  let sheetData = list.map(v => {
    return {
      原文標題: v.title,
      原文正文: v.text,
      譯文標題: v.titleTranslate,
      譯文正文: v.textTranslate,
      音訊網址: v.audioSrc,
    }
  })
...
<template>
  <div class="add-article">
    <div class="aslide">
      ...
      <div class="add" v-if="!article.title">正在新增文章...</div>
      <div class="footer">
        <div class="import">
