...
const progressTextLeft = $computed(() => {
  ...
  return '當前進度：已學' + store.currentStudyProgress + '%'
})
...
const systemPracticeText = $computed(() => {
  if (settingStore.wordPracticeMode === WordPracticeMode.Free) {
    ...
    return isSaveData
      ? '繼續' + WordPracticeModeNameMap[settingStore.wordPracticeMode]
      : '開始' + WordPracticeModeNameMap[settingStore.wordPracticeMode]
  }
})
</script>
        ...
        <template v-if="store.sdict.id">
          <div class="mt-4 space-y-2">
            ...
            <div class="text-sm flex justify-between">
              ...
              <span> {{ store.sdict?.lastLearnIndex }} / {{ store.sdict.length }} 詞</span>
            </div>
          </div>
          <div class="flex items-center mt-4 gap-4">
            ...
            <PopConfirm
              :disabled="!isSaveData"
              title="當前存在未完成的學習任務，修改會重新生成學習任務，是否繼續？"
    ...
    <div class="card flex flex-col">
      <div class="flex justify-between">
        ...
        <div class="flex gap-4 items-center">
          <PopConfirm title="確認刪除所有選中詞典？" @confirm="handleBatchDel" v-if="selectIds.length">
